...
function updateButtons() {
	chosenCounter.textContent =
		...
		(state.chosen.length === state.columns.length && state.columns.length
			? ' (все)'
			: '')
...
async function loadTables(schema) {
	...
	try {
		...
		const data = await getJSON(
			`${API_BASE}/api/v1/db/tables?schema=${encodeURIComponent(schema)}`
		)
		state.tables = parseTables(data)

		const opt = t =>
			`<option value="${t.name}" title="${titleOf(t)}">${labelOf(t)}</option>`
		tableSel.innerHTML =
			`<option value="">Выберите таблицу</option>` +
			state.tables.map(opt).join('')
		...
		el('tableError').textContent =
			'Не удалось получить список таблиц с /api/v1/db/tables. ' +
			(e.message || e)
...
async function loadColumns(schema, table) {
	...
	try {
		loader.show() // Показываем loader
		const data = await getJSON(
			`${API_BASE}/api/v1/db/columns?schema=${encodeURIComponent(
				schema
      ...
      <label class="item" title="${titleOf(c)}">
        ...
        <span style="overflow:hidden;text-overflow:ellipsis">${labelOf(
					c
				)}</span>
      </label>
    `
		...
		sortField.innerHTML =
			...
			state.columns
				.map(
					c =>
						`<option value="${c.name}" title="${titleOf(c)}">${labelOf(
							c
						)}</option>`
				)
				.join('')
		...
		el('columnsError').textContent =
			'Не удалось получить столбцы с /api/v1/db/columns. ' + (e.message || e)
...
btnDownload.addEventListener('click', async () => {
	...
	try {
		...
		btnDownload.textContent = '⬇️ Скачать'
		updateButtons()
		loader.hide() // Скрываем loader в любом случае
	}
})
...
// Показываем loader при загрузке страницы и загружаем схемы
document.addEventListener('DOMContentLoaded', function () {
	loader.show()
	loadSchemas() // Эта функция сама вызовет loader.hide() когда закончит
})
